...
for i, w in enumerate(WINS[:7]):
    ...
    guitar_onsets = [
        t for t in orig if not any(abs(t - bt) < MATCH for bt in bass)
    ]
    ...
    if guitar:
        cut = guitar - MARGIN
        print(f"win{i} last_click={last_click:.3f} guitar={guitar:.3f} bass={we:.3f} "
              f"CUT={cut:.3f} (keeps {(cut - last_click) * 1000:+.0f}ms past click)")
    else:
        cut = min(we, last_click + 0.10)
        print(f"win{i} last_click={last_click:.3f} NO guitar-only onset bass={we:.3f} "
